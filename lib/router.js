...
exports.router=function(app){
	...
	//-----------------------------------------------------
	app.get("/admin",pass.ensureAuthenticated,pass.adminIsAuthenticated,function(req,res) {
	  res.render(__dirname+'/../views/template.ejs',{title:"Admin",header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:content.adminSignIn()});
	});
	app.get("/admin_administrate",pass.ensureAuthenticated,pass.ensureAdminAuthenticated,function(req,res) {
	  res.render(__dirname+'/../views/template.ejs',{title:"Administrate",header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:content.administrate()});
	});
	...
	app.get("/profile",pass.ensureAuthenticated,function(req,res){
		res.render(__dirname+'/../views/template.ejs',{title:"Profile "+users_sessions.autorizedUsers[req.session.id].login,header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:content.profile()});
	});
	...
	app.get('/BlackJackGame',pass.ensureAuthenticated, function (req,res,next) {
		...
		if(table==undefined)
			return next();//404page
	   res.render(__dirname+'/../views/template.ejs',{title:"Black Jack - Table "+table.tableNum,header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:content.game(table)});
	});
