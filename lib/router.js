...
exports.router=function(app){
	...
	app.get("/profile",pass.ensureAuthenticated,function(req,res){
		content.profile(req.session.id).then(function(result){
			res.render(__dirname+'/../views/template.ejs',{title:"Profile "+users_sessions.autorizedUsers[req.session.id].login,header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:result});
		});
	});
