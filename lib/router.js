...
exports.router=function(app){
	...
	app.get("/admin",pass.ensureAuthenticated,pass.adminIsAuthenticated,function(req,res) {
	  res.render(__dirname+'/../views/template.ejs',{title:"Admin",header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].admin,users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:content.adminSignIn()});
	});
	...
	app.get("/",function(req,res) {
	  res.render(__dirname+'/../views/template.ejs',{title:"Main",header:parts.header(req.isAuthenticated(),users_sessions.autorizedUsers[req.session.id].admin,users_sessions.autorizedUsers[req.session.id].login),footer:parts.footer(req.isAuthenticated(),parts.chat()),content:content.main()});
	});
	...
	// 	var targetPath=__dirname+"/../static/img/avatars/"+req.user.login+".jpg";
