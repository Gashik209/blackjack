...
User.prototype.updUserGameBank=function(win){
	this.bank+=win;
	this.updUserBank();
}
User.prototype.updUserBank=function(){
        dbnedb.db.usersStat.update({userId:this.id},{$set:{bank:this.bank}}, {}, function (err, numReplaced) {
          if(err)
          	console.error(err);
        });
}
