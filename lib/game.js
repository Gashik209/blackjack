...
GameTable.prototype.initTradeRound=function(){
	...
	if(tradeStadePlayers.length==0){
		...
		return sockets.initTradeRound(hitsPlayers,this.tableNum-1,this.activeDeck);
	}
}
...
GameTable.prototype.playerX2=function(place,sessionID){
		...
		if(users_sessions.autorizedUsers[sessionID].checkAvaibleBet(this.players[place].bet))
			return;
		this.players[data.place].bet=this.players[data.place].bet*2;
...
GameTable.prototype.endRound=function(){
		...
		if(this.players[key]!=null){
			switch(this.players[key].playerStatus){
				...
				case 50:this.players[key].bet=this.players[key].bet*2;
					determinateUsersBank(this.players[key].sessionID,this.players[key].bet);
					if(config.get("globalLog"))
	...
	function determinateUsersBank(session,bet){
		if(roundPlayers[session]==undefined)
			return roundPlayers[session]=bet;
			roundPlayers[session]+=bet;
	}
	for(var key in roundPlayers){
		users_sessions.autorizedUsers[key].updUserGameBank(roundPlayers[key]);
	}

	// users_sessions. -save to DB
