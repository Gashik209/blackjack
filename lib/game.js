...
var Player=function(sessionID,place){
	...
	this.playerStatus=0;
	//player status info:
	//10-bets stage;11-bets done;12-will trade;13-hit;14-stop trade(stand);24-double;
...
GameTable.prototype.addCardToPlayer=function(player){
	if(player.place!="dealer"&&player.playerStatus!=13)
...
GameTable.prototype.initTradeRound=function(){
	...
	// console.log(tradeStadePlayers.length+"!!"+hitsPlayers.length);
	if(tradeStadePlayers.length==0&&hitsPlayers.length==0){
		setTimeout(function() {
			return this.initDealersTurn();
		}.bind(this), config.get("dealer_turn_delay"));
	}
	if(tradeStadePlayers.length==0){
	...
	for (var key in this.players) {
		if(this.players[key]!=null){
			switch(this.players[key].playerStatus){
				...
				case 51:
					if(config.get("globalLog"))
						console.log("Table № "+this.tableNum+": Player "+this.players[key].place+" WIN BLack Jack "+this.players[key].bet*3);
					break;
				case 52:
					if(config.get("globalLog"))
