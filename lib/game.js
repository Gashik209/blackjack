...
var Player=function(sessionID,place){
	...
	this.playerStatus=0;
	//player status info:
	//10-bets stage;11-bets done;12-continue trade;14-stop trade(stand);24-player call double;
...
GameTable.prototype.sitOnPlace=function(place,sessionID){
		...
		if(this.players[place]==null){
			...
			if(config.get("globalLog"))
				console.log("Table № "+this.tableNum+": "+users_sessions.autorizedUsers[sessionID].login+" sit on "+place);
		}
...
GameTable.prototype.leaveTable=function(sessionID){
		if(config.get("globalLog"))
			console.log("Table № "+this.tableNum+": "+"Player "+users_sessions.autorizedUsers[sessionID].login+" leave table "+this.tableNum);
...
GameTable.prototype.checkGameStatus=function(){
		if(this.countPlayersInGame()>0&&this.gameStatus==0){//first player
			this.initBetsStage();
		}
		else if(this.countPlayersInGame()>0&&this.gameStatus==3){//end round and start new round
			this.initBetsStage();
		}
		else if (this.countActivePlayersInGame()==0&&this.gameStatus!=0){//restart game (all active players leave places)
			this.gameStatus=0;
			if(config.get("globalLog"))
				console.log("Table № "+this.tableNum+": All players leave table, round end;");
...
GameTable.prototype.chipToBank=function(data,sessionID){
		...
		if(users_sessions.autorizedUsers[sessionID].checkAvaibleBet(data.chipcost))
			return;
		if(this.players[data.place].sessionID==sessionID)
			this.players[data.place].bet+=data.chipcost;
}
GameTable.prototype.betsDone=function(data){
		if(this.gameStatus!=1)
			return;
		if(this.players[data].playerStatus!=10||!this.players[data].bet)
			return;
		if(config.get("globalLog"))
			console.log("Table № "+this.tableNum+": Player "+data+" made bet;");
	...
	for(var key in this.players){
		if(this.players[key]!=null){
			if(this.players[key].playerStatus==12)
				activePlayers.push(this.players[key].place);
		}
	}
	if(activePlayers.length==0)
		return this.initDealersTurn();
	sockets.initTradeRound(activePlayers,this.tableNum-1);//сделать чтобы до конца раунда не отправлялся новая инициализация заунда на клиенте
}
GameTable.prototype.definitionPlayerWinLoose=function(player){
	...
	if(player.cardScore<21&&player.cards.length==config.get("players_max_card_limit")){
		...
		if(config.get("globalLog"))
			console.log("Table № "+this.tableNum+": Player  "+player.place+" bust;");
	}
	else if(player.cardScore==21){
...
GameTable.prototype.initDealersTurn=function(){
	if(config.get("globalLog"))
		console.log("Table № "+this.tableNum+": Dealers turns;");
	for (var key in this.players) {
		if(this.players[key]!=null){
			if(this.players[key].playerStatus==14||this.players[key].playerStatus==23||this.players[key].playerStatus==24||this.players[key].playerStatus==25){
				sockets.openSecondDealerCard(this.tableNum-1,this.dealer.cards[1][0],this.dealer.cards[1][1]);
				while(this.dealer.cardScore<17){
					this.addCardToPlayer(this.dealer);
				}
				if(config.get("globalLog"))
					console.log("Table № "+this.tableNum+": Dealers have "+this.dealer.cardScore);
			}
		}
	}
	...
	for (var key in this.players) {
		if(this.players[key]!=null){
			switch(this.players[key].playerStatus){
				...
				case 50:
					if(config.get("globalLog"))
						console.log("Table № "+this.tableNum+": Player"+this.players[key].place+" WIN "+this.players[key].bet*2);
					break;
				case 51:
					if(config.get("globalLog"))
						console.log("Table № "+this.tableNum+": Player"+this.players[key].place+" WIN BLack Jack "+this.players[key].bet*2.5);
					break;
				case 52:
					if(config.get("globalLog"))
				...
				case 53:
					if(config.get("globalLog"))
						console.log("Table № "+this.tableNum+": Player"+this.players[key].place+" loose ");
					break;
