...
GameTable.prototype.standFromPlace=function(place,sessionID){
		...
		if(this.players[place].sessionID!=sessionID)
			return;
		if(config.get("globalLog"))
			console.log("Player "+sessionID+" leave place "+place);
...
GameTable.prototype.checkGameStatus=function(){
		if(this.countPlayersInGame()>0&&this.gameStatus==0){
			this.initBetsStage();
		}
		else if (this.countActivePlayersInGame()==0&&this.gameStatus!=0){
			this.gameStatus=0;
			this.dealer.cards=[];
		...
		else if(this.countActivePlayersInGame()>0&&this.gameStatus==1){
			this.checkAllPleyerStatusInBatsStage();
		}
		else if(this.countActivePlayersInGame()>0&&this.gameStatus==2){
			this.initTradeRound();
		}	
}
...
GameTable.prototype.betsDone=function(data){
		if(this.gameStatus!=1)
			return;
		if(this.players[data]==null||data==undefined){
			this.checkGameStatus();
...
GameTable.prototype.initTradeRound=function(){
	...
	for(var key in this.players){
		if(this.players[key]!=null){
			if(this.players[key].playerStatus==12&&this.players[key].cards.length>1)
				activePlayers.push(this.players[key]);
		}
	}
	if(activePlayers.length==0)
		return this.initDealersTurn();
	sockets.initTradeRound(activePlayers,this.tableNum-1);
}
