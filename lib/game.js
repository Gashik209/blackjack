...
GameTable.prototype.leaveTable=function(sessionID){
		...
		// users_sessions.autorizedUsers[sessionID].ingamePlace.forEach(function(item){
		// 	this.players[item]=null;
		// }.bind(this));
...
GameTable.prototype.initBetsStage=function(){
	...
	// for (var key in this.players) {
	// 	if(this.players[key]!=null)
	// 		// users_sessions.autorizedUsers[this.players[key].sessionID].updPlayerBank();
	// }
	sockets.initTableBets(this.tableNum-1);
}
...
GameTable.prototype.chipToBank=function(chipcost,sessionID){
	console.log(chipcost);
		if(this.players[sessionID].bet==undefined)
			this.players[sessionID].bet=0;
